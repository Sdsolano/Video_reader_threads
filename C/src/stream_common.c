...
int decodeAllHeaders(int respac, struct streamstate *s, enum streamtype type) {
  ...
  if (respac == 1 && (!s->headersRead) && s->strtype != TYPE_VORBIS) {
    ...
    if (res != TH_ENOTFORMAT) {
      ...
      printf("Initializing theora decoder\n");
	...
	// BEGIN your modification HERE
        ...
        assert(pthread_create(&thread_draw, NULL, draw2SDL, (void*)(long long int)s->serial) == 0);
        theora2sdlthread = thread_draw;
...
void cleanup_streams() {
    ...
    HASH_ITER(hh, theorastrstate, s, tmp) {
        HASH_DEL(theorastrstate, s);
        ogg_stream_clear(&s->strstate);
        th_decode_free(s->th_dec.ctx);
        th_comment_clear(&s->th_dec.comment);
        th_info_clear(&s->th_dec.info);
        free(s);
    }
    pthread_mutex_unlock(&mutex_hashmap);
    ...
    HASH_ITER(hh, vorbisstrstate, s, tmp) {
        HASH_DEL(vorbisstrstate, s);
        ogg_stream_clear(&s->strstate);
        vorbis_block_clear(&s->vo_dec.block);
        vorbis_dsp_clear(&s->vo_dec.dsp);
        vorbis_comment_clear(&s->vo_dec.comment);
        vorbis_info_clear(&s->vo_dec.info);
        free(s);
    }
    pthread_mutex_unlock(&mutex_hashmap);
}